...
class MultiHeadAttentionRelative(tf.keras.layers.Layer):
    ...
    def build(self, input_shape: tf.Tensor):
        ...
        if self.for_saliency:
            ...
            self.fiducial_bias = self.add_weight(
                name="fiducial_bias",
                shape=[self.num_heads, num_query_elements, num_key_elements],
                initializer=self.bias_initializer,
                trainable=False,
            )
    ...
    def call(
        ...
        key_edge = tf.gather(self.embedding_key, relative_pos, axis=-1)
        value_edge = tf.gather(self.embedding_value, relative_pos, axis=-1)

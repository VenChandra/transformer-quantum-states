...
def gradient_attention_score(
    ...
    orig_scaling = [
        tf.Variable(decoder.layers[idx].weights[-2]) for idx in range(1, num_layers)
    ]
            ...
            for idx in range(seq_length - 1):
                for m in range(1, riemann_steps + 1):
                    ...
                    attention_saliency_tensor[
                        layer_idx, batch_idx, :, idx + 1, : idx + 1
                    ] += (
                        tf.nn.relu(grads)
                        * attention_weights[layer_idx][batch_idx, :, idx + 1, : idx + 1]
                    )

...
def labels_to_spins(vocab_length: int, labels: tf.Tensor) -> tf.Tensor:
    ...
    spins_from_labels = tf.gather(
        np.array(spin_vocab, dtype=int), labels.astype(int), axis=0
    )
...
class TransformerDecoder(tf.keras.Model):
    ...
    Attributes:
        ...
        for_saliency: bool = False, whether to include fiducial weights in attention layer
            for attention gradient computation
        batch_size: int = None, only needs to be specified when computing attention saliency
    """
    ...
    def __init__(
        ...
        self.dropout_rates = (dropout, attention_dropout)
        self.trainable_positional_encoding = trainable_positional_encoding
        self.for_saliency = for_saliency
        ...
        self.attention_layers = [
            MultiHeadAttentionRelative(
                ...
                for_saliency=self.for_saliency,
    ...
    def evaluate_gradients(
        ...
        for _ in range(reps):
            ...
            e_loc_real_part, e_loc_imag_part = local_energy_function(
                decoder=self,
                configuration=labels_to_spins(
                    labels=samples, vocab_length=self.dictionary_size
                ),
            )
